...
def evaluate_double_biceps(left_angle, right_angle, left_elbow_height, right_elbow_height, left_shoulder_height, right_shoulder_height):
    min_elbow_height = 0.9  # Definir um limite mínimo para a altura relativa dos cotovelos
    max_elbow_height = 1.1  # Definir um limite máximo para a altura relativa dos cotovelos
    
    if left_elbow_height is None or right_elbow_height is None:
        return "Cotovelos não estão visíveis."
    
    if left_elbow_height < left_shoulder_height * min_elbow_height:
        return "Levante o cotovelo esquerdo mais."
    elif left_elbow_height > left_shoulder_height * max_elbow_height:
        return "Abaixe o cotovelo esquerdo mais."
    
    if right_elbow_height < right_shoulder_height * min_elbow_height:
        return "Levante o cotovelo direito mais."
    elif right_elbow_height > right_shoulder_height * max_elbow_height:
        return "Abaixe o cotovelo direito mais."
    
    if left_angle > 150 and right_angle > 150:
        return "Posição correta - Excelente postura!"
    elif left_angle > 120 and right_angle > 120:
        return "Posição moderada - Ajuste ligeiramente os braços."
    else:
        return "Posição incorreta - Reajuste completamente a pose."
...
while cap.isOpened():
    ...
    if results.pose_landmarks:
        ...
        left_elbow_height = elbow_left[1] if elbow_left else None
        right_elbow_height = elbow_right[1] if elbow_right else None
